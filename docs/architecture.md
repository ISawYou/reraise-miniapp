...
Сайт `re-raise.ru` (контейнер на VPS) работает с `DATABASE_PROVIDER=postgres`.
Backfill данных завершён (см. раздел 4), Repository Layer полностью реализован для
PostgreSQL по всем 11 доменам. Vercel-деплой (Telegram Mini App) остаётся на
Supabase — это два независимых деплоя одного и того же кода с разным окружением, не
переходное состояние одной миграции (см. раздел 8).
...
## 8. Mini App и сайт — сознательное разделение

Это два независимых деплоя **одного и того же кода**, с разной конфигурацией
окружения:

| | re-raise.ru | Telegram Mini App |
|---|---|---|
| Хостинг | VPS, Docker (`docker compose`) | Vercel |
| `DATABASE_PROVIDER` | `postgres` | не задан (Supabase) |
| Точка входа пользователя | обычный браузер, Email OTP или Telegram initData через nginx | Telegram WebView, кнопка меню бота |
| Storage | локальная ФС VPS | Supabase Storage |

Telegram Bot API подтверждает разделение: `getWebhookInfo` и `getChatMenuButton`
указывают на `reraise-miniapp.vercel.app` — отдельный от `re-raise.ru` домен и деплой.
Ничего в этом документе, относящееся к PostgreSQL/локальному Storage/Node
Middleware, не затрагивает Vercel-деплой — это разные процессы, разные переменные
окружения, разный код выполняется идентично, но с разными зависимостями на выходе.

Это не переходное состояние одной незавершённой миграции, а осознанная точка
разделения: сайт полностью независим от Supabase Database, Mini App полностью
зависит. Оба варианта поддерживаются одной кодовой базой благодаря Repository Layer.
...
2. **Telegram Mini App (деплой на Vercel)** — целиком на Supabase (БД + Storage),
   не участвует в миграции на PostgreSQL, отдельный процесс с отдельным окружением
   (раздел 8).
